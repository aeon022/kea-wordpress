function dancepad_fluids(){window.ga=window.ga||function(){(ga.q=ga.q||[]).push(arguments)};ga.l=+new Date();ga("create","UA-105392568-1","auto");ga("send","pageview");class Dance_Fluids{constructor(canvas,config){this.fluidElement=canvas.closest(".dan-fluids");this.init(canvas,config)}
init(canvas,config){resizeCanvas(canvas);function pointerPrototype(){this.id=-1;this.texcoordX=0;this.texcoordY=0;this.prevTexcoordX=0;this.prevTexcoordY=0;this.deltaX=0;this.deltaY=0;this.down=!1;this.moved=!1;this.color=[30,0,300]}
let pointers=[];let splatStack=[];pointers.push(new pointerPrototype());const{gl,ext}=getWebGLContext(canvas);if(isMobile()){config.DYE_RESOLUTION=512;config.initialAnim=!1}
...
function normalizeTexture(texture,width,height){let result=new Uint8Array(texture.length);let id=0;for(let i=height-1;i>=0;i--){for(let j=0;j<width;j++){let nid=i*width*4+j*4;result[nid+0]=clamp01(texture[id+0])*255;result[nid+1]=clamp01(texture[id+1])*255;result[nid+2]=clamp01(texture[id+2])*255;result[nid+3]=clamp01(texture[id+3])*255;id+=4}}
return result}
...
updateKeywords();initFramebuffers();if(config.initialAnim){multipleSplats(parseInt(Math.random()*20)+5)}
...
if(config.trigger==="hover"){this.fluidElement.addEventListener("mouseover",(e)=>{let rect=canvas.getBoundingClientRect();let posX=scaleByPixelRatio(e.clientX-rect.left);let posY=scaleByPixelRatio(e.clientY-rect.top);let pointer=pointers.find((p)=>p.id==-1);if(pointer==null)pointer=new pointerPrototype();updatePointerDownData(pointer,-1,posX,posY)});this.fluidElement.addEventListener("mousemove",(e)=>{let pointer=pointers[0];let rect=canvas.getBoundingClientRect();let posX=scaleByPixelRatio(e.clientX-rect.left);let posY=scaleByPixelRatio(e.clientY-rect.top);updatePointerMoveData(pointer,posX,posY)});this.fluidElement.addEventListener("mouseout",()=>{updatePointerUpData(pointers[0])})}else if(config.trigger==="click"){this.fluidElement.addEventListener("mousedown",(e)=>{let rect=canvas.getBoundingClientRect();let posX=scaleByPixelRatio(e.clientX-rect.left);let posY=scaleByPixelRatio(e.clientY-rect.top);let pointer=pointers.find((p)=>p.id==-1);if(pointer==null)pointer=new pointerPrototype();updatePointerDownData(pointer,-1,posX,posY)});this.fluidElement.addEventListener("mousemove",(e)=>{let pointer=pointers[0];if(!pointer.down)return;let rect=canvas.getBoundingClientRect();let posX=scaleByPixelRatio(e.clientX-rect.left);let posY=scaleByPixelRatio(e.clientY-rect.top);updatePointerMoveData(pointer,posX,posY)});window.addEventListener("mouseup",()=>{updatePointerUpData(pointers[0])})}
this.fluidElement.addEventListener("touchstart",(e)=>{e.preventDefault();const touches=e.targetTouches;while(touches.length>=pointers.length)pointers.push(new pointerPrototype());let rect=canvas.getBoundingClientRect();for(let i=0;i<touches.length;i++){let posX=scaleByPixelRatio(touches[i].clientX-rect.left);let posY=scaleByPixelRatio(touches[i].clientY-rect.top);updatePointerDownData(pointers[i+1],touches[i].identifier,posX,posY)}});this.fluidElement.addEventListener("touchmove",(e)=>{e.preventDefault();const touches=e.targetTouches;let rect=canvas.getBoundingClientRect();for(let i=0;i<touches.length;i++){let pointer=pointers[i+1];if(!pointer.down)continue;let posX=scaleByPixelRatio(touches[i].clientX-rect.left);let posY=scaleByPixelRatio(touches[i].clientY-rect.top);updatePointerMoveData(pointer,posX,posY)}},!1);this.fluidElement.addEventListener("touchend",(e)=>{const touches=e.changedTouches;for(let i=0;i<touches.length;i++){let pointer=pointers.find((p)=>p.id==touches[i].identifier);if(pointer==null)continue;updatePointerUpData(pointer)}});window.addEventListener("keydown",(e)=>{if(e.code==="KeyP")config.PAUSED=!config.PAUSED;if(e.key===" ")splatStack.push(parseInt(Math.random()*20)+5);});function updatePointerDownData(pointer,id,posX,posY){pointer.id=id;pointer.down=!0;pointer.moved=!1;pointer.texcoordX=posX/canvas.width;pointer.texcoordY=1.0-posY/canvas.height;pointer.prevTexcoordX=pointer.texcoordX;pointer.prevTexcoordY=pointer.texcoordY;pointer.deltaX=0;pointer.deltaY=0;pointer.color=generateColor()}
...
function generateColor(){if(!config.COLORFUL){return{r:config.customColor.r*0.15,g:config.customColor.g*0.15,b:config.customColor.b*0.15}}
...
return hash}}}
var fluids=document.querySelectorAll(".dan-fluids");fluids.forEach((fluid)=>{fluid.style.pointerEvents='all';let canvas=fluid.querySelector(".dan-fluids__canvas");let fluidStyle=window.getComputedStyle(fluid);let backgroundColor=fluidStyle.backgroundColor;let rgb=backgroundColor.match(/\d+/g).map(Number);let customColor=fluid.getAttribute("data-custom-color");let customRGB=null;if(customColor){customRGB={r:parseInt(customColor.slice(1,3),16)/255,g:parseInt(customColor.slice(3,5),16)/255,b:parseInt(customColor.slice(5,7),16)/255}}
const isCoarsePointer=window.matchMedia('(pointer: coarse)').matches;const disableEntranceTouch=fluid.getAttribute("data-disable-entrance-touch")==="1";const shouldShowInitialAnim=fluid.getAttribute("data-initial-anim")==="1"&&!(isCoarsePointer&&disableEntranceTouch);let config={customColor:customRGB,trigger:fluid.getAttribute("data-trigger"),initialAnim:shouldShowInitialAnim,SIM_RESOLUTION:128,DYE_RESOLUTION:1024,CAPTURE_RESOLUTION:512,DENSITY_DISSIPATION:parseFloat(fluid.getAttribute("data-density-diffusion")),VELOCITY_DISSIPATION:parseFloat(fluid.getAttribute("data-velocity-diffusion")),PRESSURE:parseFloat(fluid.getAttribute("data-pressure")),PRESSURE_ITERATIONS:parseFloat(fluid.getAttribute("data-pressure-iterations")),CURL:parseFloat(fluid.getAttribute("data-vorticity")),SPLAT_RADIUS:parseFloat(fluid.getAttribute("data-splat-radius")),SPLAT_FORCE:parseFloat(fluid.getAttribute("data-splat-force")),SHADING:fluid.getAttribute("data-shading")=="1"||fluid.getAttribute("data-shading")=="true",COLORFUL:fluid.getAttribute("data-colorful")=="1"||fluid.getAttribute("data-colorful")=="true",COLOR_UPDATE_SPEED:10,PAUSED:!1,BACK_COLOR:{r:rgb[0],g:rgb[1],b:rgb[2]},TRANSPARENT:!1,BLOOM:fluid.getAttribute("data-bloom")=="1"||fluid.getAttribute("data-bloom")=="true",BLOOM_ITERATIONS:parseFloat(fluid.getAttribute("data-bloom-iterations")),BLOOM_RESOLUTION:256,BLOOM_INTENSITY:parseFloat(fluid.getAttribute("data-bloom-intensity")),BLOOM_THRESHOLD:parseFloat(fluid.getAttribute("data-bloom-threshold")),BLOOM_SOFT_KNEE:0.7,SUNRAYS:fluid.getAttribute("data-sunrays")=="1",SUNRAYS_RESOLUTION:196,SUNRAYS_WEIGHT:parseFloat(fluid.getAttribute("data-sunrays-weight")),};new Dance_Fluids(canvas,config)})}