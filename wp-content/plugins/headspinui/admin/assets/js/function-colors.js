...
document.addEventListener('headspinready', function () {
	...
	window.tippyColorpicker = new ReinventedColorWheel({
		...
		onChange: function (color) {
			...
			console.log(Alpine.store('colorWheel').tcpValue);
	...
	function syncColors() {
		...
		//refresh preview
		if (Alpine.store('project').schemaEditorTab == 'radix') {
			console.log(Alpine.store('project').brandValues);
		}
		else if (Alpine.store('project').schemaEditorTab == 'custom') {
			console.log(Alpine.store('project').customColors);
		}
	}
	//syncColors();
});
