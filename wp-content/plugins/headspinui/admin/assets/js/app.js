...
function generateEmphasisColor(palette, name, data) {

	let mainColor = data['mainColor'];
	let bg = palette[4].output;
	let fg = mainColor || palette[8].output;
	let fgAlt = palette[10].output;
	let contrast = chroma.contrast(fg, bg);
	if (contrast < 4.5) {
		fg = fgAlt;
	}
	console.log({ palette, name, data, mainColor, fg })
	console.log({ contrast })
	return `--hcl-${name}-emphasis: ${fg};`;
}
function generateMainColor(data, name) {
	return `--hcl-${name}-main: ${data.mainColor};`;
}
function generateColorCSS() {
	...
	buffer += generateMainColor(Alpine.store('pd').brandColorSchema.data, 'brand');
	Alpine.store('pd').brandColorSchema.data[light].forEach((s) => {
		buffer += `\n\t--${clrPrefix}${name}-${s.step}${clrSufix}: ${s.output};`;
	});
	buffer += generateEmphasisColor(Alpine.store('pd').brandColorSchema.data[light], 'brand', Alpine.store('pd').brandColorSchema.data);
	...
	buffer += `\n\t--${clrPrefix}on-${name}${clrSufix}: ${onColor};`;
	buffer += generateMainColor(Alpine.store('pd').neutralColorSchema.data, 'neutral');
	Alpine.store('pd').neutralColorSchema.data[light].forEach((s) => {
		buffer += `\n\t--${clrPrefix}${name}-${s.step}${clrSufix}: ${s.output};`;
	});
	buffer += generateEmphasisColor(Alpine.store('pd').neutralColorSchema.data[light], 'neutral', Alpine.store('pd').neutralColorSchema.data);
	...
	Alpine.store('pd').colorSchemas.forEach((schema) => {
		if (!schema.presetColor || (schema.presetColor && schema.enabled)) {
			...
			buffer += `\n\t--${clrPrefix}on-${name}${clrSufix}: ${onColor};`;
			buffer += generateMainColor(schema.data, name);
			if (Alpine.store('pd').colorContrastMethod == 'wcag_20') {
				onColor = '#ffffff';
				if (
					chroma.contrast(
			...
			buffer += generateEmphasisColor(schema.data[light], name, schema.data);
	...
	*/
	...
	buffer += generateEmphasisColor(Alpine.store('pd').brandColorSchema.data[dark], 'brand', Alpine.store('pd').brandColorSchema.data);
	...
	buffer += generateEmphasisColor(Alpine.store('pd').neutralColorSchema.data[dark], 'neutral', Alpine.store('pd').neutralColorSchema.data);
	name = Alpine.store('pd').neutralColorSchema.name.toLowerCase();
	onColor = Alpine.store('pd').neutralColorSchema.data.onColor;
	if (true) {
		onColor = '#ffffff';
	...
	Alpine.store('pd').colorSchemas.forEach((schema) => {
		if (!schema.presetColor || (schema.presetColor && schema.enabled)) {
			if (schema.enabled) {
				name = schema.name.toLowerCase();
				//onColor = schema.data.onColor;
				buffer += generateEmphasisColor(schema.data[dark], name, schema.data);
