...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const dropdownMounts = ['.v-menu__content', '.bricks-control-popup', '.v-popover']
    const modalTriggers = [
    ...
    function showApp(e) {
        ...
        if (false) {
            ...
            else if (appState.activeInput.closest('.v-popover')) {
                appState.activeInput.closest('.breakdance-control-wrapper').insertAdjacentHTML('afterend', appState.html);
            }
            else {
                appState.activeInput.closest('.v-menu__content').insertAdjacentHTML('afterend', appState.html);
            }
        ...
        mountApp();
        if (isEnabledMainColors(appData) && doc.querySelector('#headspin-tokenWP')) doc.querySelector('#headspin-tokenWP').setAttribute("data-maincolors", "enabled")
...
*/
    ...
    function SectionColorPalette(color, name, theme) {
        ...
        console.log(isEnabledMainColorsColor(color))
        if (isEnabledMainColorsColor(color)) {
            activeElementSelectorMap.forEach(selector => {
                if (document.querySelector(selector)) {
                    let propMain = `--hcl-${name}-main`;
                    let propEmphasis = `--hcl-${name}-emphasis`;
                    let computedStyleMain = window.getComputedStyle(document.querySelector(selector));
                    let localScopeCSSValueMain = computedStyleMain.getPropertyValue(propMain).trim();

                    let computedStyleEmphasis = window.getComputedStyle(document.querySelector(selector));
                    let localScopeCSSValueEmphasis = computedStyleMain.getPropertyValue(propEmphasis).trim();
                    styleEmphasis = `style="--_local: ${localScopeCSSValueMain};"`
                    styleMain = `style="--_local: ${localScopeCSSValueMain};"`
                }
            })
            console.log('--MAIN ENABLED')


            html += `<button class="tokenwp-button tokenwp-solid tokenwp-maincolor tokenwp-button-colorpalette" data-type="color-palette" data-embed="--hcl-${name}-main" ${styleMain}><div class="tokenwp-transparent-preview"></div></button>`;
            html += `<button class="tokenwp-button tokenwp-solid tokenwp-emphasiscolor tokenwp-button-colorpalette" data-type="color-palette" data-embed="--hcl-${name}-emphasis" ${styleEmphasis}><div class="tokenwp-transparent-preview"></div></button>`;
...
<svg>
    ...
    function isEnabledMainColors(data) {
        console.log({ data })
        if (data?.colors?.brand.data?.mainColor) {
            return true;
        }
        else return false;
    }
    function isEnabledMainColorsColor(color) {
        if (color?.data?.mainColor) {
            return true;
        }
        else return false;
    }
